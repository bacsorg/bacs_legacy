...
class CProblem {
 ...
 public:
  ...
  bool run_tests(cstr run_cmd, cstr src_lang, int &result, double &max_time,
                 double &max_memory, int &test_num_failed, string *info,
                 bool acm = true, string &test_results = nstr);
  bool run_test(const CTest &tt, cstr run_cmd, cstr src_lang, int &result,
                double &time_used, double &memory_used, string *info);
};
...
inline void ping(ping_type type, const std::string &submit_id = std::string(),
                 int test = -1, const std::string &problem_id = std::string(),
                 std::string result = std::string()) {
  if (!cf_ping_uri.empty()) {
    str_replace(result, " ", "_");
    system(format("curl --silent --output /dev/null "
                  "'%s?type=%d&submit_id=%s&test=%d&problem_id=%s&result=%s'",
                  cf_ping_uri.c_str(), type, submit_id.c_str(), test,
                  problem_id.c_str(), result.c_str())
               .c_str());
  }
}
