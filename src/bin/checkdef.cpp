...
enum error_code { OK = 0, WA = 2, PE = 4, UN = 100 };
...
int main(int argc, char **argv) {
  assert(run_tests());
  try {
    if (argc != 3 + 1) throw std::invalid_argument("wrong argument count");
    ...
    std::cerr << "File error: [" << typeid(e).name() << "] " << e.what()
              << std::endl;
    return UN;
  } catch (std::exception &e) {
    std::cerr << "Error: [" << typeid(e).name() << "] " << e.what()
              << std::endl;
    std::cerr << "Usage: checkdef <input file> <output file generated by "
                 "solution> <correct output file>"
              << std::endl
              << std::endl;
    ...
    std::cerr << "\t" << OK << " - OK" << std::endl
              ...
              << "\t" << UN << " - Invalid parameters or check error"
              << std::endl;
    return UN;
  }
}
