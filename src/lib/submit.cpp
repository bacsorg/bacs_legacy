...
CSubmit::CSubmit(cstr _sid) {
  ...
  test_num_failed = -1;
  DBRow rr =
      db_qres(format("select problem_id, lang, solution, need_info, acm from "
                     "submit where submit_id = %s",
                     sid.c_str()));
  if (rr.empty()) {
    not_found = true;
  } else {
...
bool CSubmit::compile_and_test() {
  ...
  log.add(
      format("Finished testing [%s]", sid.c_str()),
      log.gen_data("Result", status_to_str(status), "Failed on test",
                   i2s(test_num_failed), "Time used", format("%.3lf", max_time),
                   "Memory used", format("%.3lf", max_memory)));
...
bool CSubmit::test() {
  ...
  if (!problem.init(pid, sid)) {
    log.add("Error initializing problem data!",
            log.gen_data("Problem ID", pid));
    status = ST_INVALID_PROBLEM;
    return false;
  }
  int st = ST_SERVER_ERROR;
  int no_ml = lcfgi(str_lowercase(lang) + ".no_memory_limit");
  if (no_ml) problem.set_no_memory_limit();
  bool res = problem.run_tests(run_cmd, lang, st, max_time, max_memory,
                               test_num_failed, need_info ? (&info) : NULL, acm,
                               test_results);
  status = st;
...
bool CSubmit::cleanup() {
  ...
  if (run(cs, NULL, true, "", err, cf_compiler_timeout) != RUN_OK) {
    log.add("Error: error in cleanup process!",
            log.gen_data("Command string", cs, "Output", err));
    return false;
  }
  return true;
}
...
bool CSubmit::store_result() {
  ...
  string s_info =
      need_info ? format(", info = '%s'", db.escape(info).c_str()) : "";
  if (acm)
    return db_query(format(
        "update submit set result = %li, test_num_failed = %s, max_time_used = "
        "%lf, "
        "max_memory_used = %lf%s where submit_id = %s",
        status, (test_num_failed < 0) ? "NULL" : i2s(test_num_failed).c_str(),
        max_time, max_memory, s_info.c_str(), sid.c_str()));
  else
    return db_query(
        format("update submit set result = %li, school_result = '%s'"
               " , test_num_failed=%s, max_time_used = %lf,  max_memory_used = "
               "%lf%s where submit_id = %s",
               status, test_results.c_str(),
               (test_num_failed < 0) ? "NULL" : i2s(test_num_failed).c_str(),
               max_time, max_memory, s_info.c_str(), sid.c_str()));

  return true;
}

bool CSubmit::security_check() { return true; }
