...
CDBEngine::CDBEngine() { connection = NULL; }
...
bool CDBEngine::connect() {
  ...
  if (!mysql_real_connect(connection, cfg("db.host").c_str(),
                          cfg("db.user").c_str(), cfg("db.pass").c_str(),
                          cfg("db.database").c_str(), cfgi("db.port"), NULL,
                          0)) {
...
string CDBEngine::escape(cstr s) {
  ...
  mysql_real_escape_string(connection, buf, s.c_str(),
