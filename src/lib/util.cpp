...
string vformat(const char *q, va_list args) {
  vsnprintf(big_buf, MAX_BIG_BUF, q,
            args);  // TODO will not write more than MAX_BIG_BUF
...
bool find_files(FileList &list, cstr fn, cstr ext) {
  ...
  while ((dirp = readdir(dp)) != NULL) {
    ...
    if ((int)nfn.find(ext) < 0) continue;
    if (file_exists(nfn)) list.push_back(nfn);
  }
  closedir(dp);
  return true;
  /*
...
bool delete_file(cstr filename) { return remove(filename.c_str()) != 0; }

void chdir(cstr dir) { ::chdir(dir.c_str()); }
